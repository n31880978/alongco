...
import { mapPaymentStatus, paymentMethodName } from '@/lib/payments/razorpay/orders'
import { mapRefundStatus } from '@/lib/payments/razorpay/refunds'

/**
 * The Razorpay webhook. CLAUDE.md §3.3, §3.4 and PRD §6.6.
 *
 * This is the ONLY thing in the system that sets a booking to `confirmed`. The
 * browser handshake after checkout is a UX signal and never touches state.
 *
 ...
 *      webhook_events — Razorpay retries, and it must confirm exactly once.
...
export async function POST(request: NextRequest) {
  // Must be the exact bytes Razorpay signed. Parsing first and re-serialising
  ...
  const eventId = webhookEventId(request.headers.get(EVENT_ID_HEADER), payload)
  if (!eventId) {
  ...
  if (claimError) {
    // 23505 = unique violation: already seen. Acknowledge so Razorpay stops.
    if ((claimError as { code?: string }).code === '23505') {
      return NextResponse.json({ status: 'duplicate' }, { status: 200 })
    }
    reportWebhookFailure('could not record webhook event', { eventId }, claimError)
    // 500 so Razorpay retries — losing the event is worse than a retry.
  ...
  try {
    ...
    // A payment Razorpay captured that we failed to apply: she has paid and her
    // booking is not confirmed. This is the page-someone case.
...
async function handleEvent(supabase: Service, payload: RazorpayWebhookPayload) {
  const event = payload.event ?? ''

  if (event === 'payment.captured') return handlePaymentCaptured(supabase, payload)
  if (event === 'payment.failed') return handlePaymentFailed(supabase, payload)
  if (event.startsWith('refund.')) return handleRefundStatus(supabase, payload)

  // Anything else — including payment.authorized, which is money held but not
  // taken — is recorded and acknowledged, not acted on. Confirming a booking on
  // an authorisation would commit a companion's hour against funds that can
  // still expire back to the customer.
}
...
async function handlePaymentCaptured(
  ...
  if (!orderId || !payment) throw new Error('payment capture without an order')
  ...
  // Guard against a payload claiming a different amount than we asked for.
  // Razorpay reports integer paise, so this is an exact comparison with no
  // rounding step that could decide it wrongly.
  if (payment.amount !== paymentRow.amount_paise) {
  ...
  const { error: paymentUpdateError } = await supabase
    ...
    .update({
      ...
      method: paymentMethodName(payment.method),
  ...
  if (statusError) {
    ...
    // acknowledges the webhook rather than looping Razorpay forever, and pages
...
async function handlePaymentFailed(supabase: Service, payload: RazorpayWebhookPayload) {
  const payment = payload.payload?.payment?.entity
  const orderId = payment?.order_id
  ...
  const { error: paymentUpdateError } = await supabase
    ...
    .update({
      ...
      provider_payment_id: payment?.id ?? null,
      method: paymentMethodName(payment?.method),
      // The provider's own description, not a raw payload dump (§9).
      failure_reason: payment?.error_description ?? 'failed',
    })
    .eq('payment_provider', PROVIDER)
    .eq('provider_order_id', orderId)
...
async function handleRefundStatus(supabase: Service, payload: RazorpayWebhookPayload) {
  ...
  // `receipt` is the refund_reference we generated and stored before calling
  // out, which is what ties this event back to our row.
  const reference = refund?.receipt
